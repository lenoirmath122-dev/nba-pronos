...
-- reprise post-audit, GAPS_OUVERTS.md, 25/08/2026) -- même patron que
-- structured_roster_count (migration 20260825120000) : nouvelle colonne
-- jsonb dédiée + nouveau paramètre sur update_bet_structuration().
--
-- Contrairement à ROSTER_COUNT, ce bet_subject centre sur UN SEUL joueur
-- (structured_player_id/structured_player_name existants suffisent pour
-- l'identifier) -- structured_superlative ne porte donc QUE la stat
-- comparée : { stat: string }. p_threshold/p_comparison restent NULL pour
-- ce type de pari (probabilité DIRECTE, même principe que dd/td) --
-- volontaire : ça évite que resolveCalculableBets() (le résolveur PLAYER
-- de base, qui filtre seulement sur structured_player_id non-null, sans
-- exclure les autres bet_subject) puisse un jour le résoudre à tort avec
-- la mauvaise formule -- computeOutcome() renvoie null dès que threshold/
-- comparison sont absents pour une stat comptée, donc ce résolveur skip
-- proprement au lieu de trancher, laissant resolveCalculableSuperlativeBets()
-- (lib/ai/resolveCalculableBets.ts) faire le vrai calcul.
alter table bets add column structured_superlative jsonb;
...
-- ============================================================================
-- FIN.
-- Verif post-push : soumettre un pari "meilleur marqueur" doit remplir
-- structured_superlative en base -- necessite le deploiement Vercel
-- (structureAndScoreBet.ts transmet le nouveau parametre a tous ses appels
-- RPC, y compris les branches existantes avec p_structured_superlative=null).
-- ============================================================================
