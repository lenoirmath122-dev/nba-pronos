...
9. **Transferts hors Espace économique européen** : identifiés à date — **Anthropic** (API Claude, société américaine, reçoit le texte libre des paris pour structuration IA) et potentiellement **Vercel** (société américaine, bien que le calcul soit déployé en région `dub1`/Dublin) selon la localisation réelle de leurs sous-traitants internes. Décision retenue : le flux vers Anthropic est conservé (pas de changement d'architecture), documenté dans la politique de confidentialité. **DPA/garanties de transfert Anthropic vérifiés (03/09/2026)**, sur les sources publiques officielles :
   - Commercial Terms of Service (`anthropic.com/legal/commercial-terms`) : *"Data submitted through the Services will be processed in accordance with the [DPA]... which is incorporated into these Terms by reference."* S'applique à tout usage de l'API (facturation à l'usage comprise, pas réservé aux comptes Enterprise) — confirmé par la doc "Commercial Terms govern... API access" incluant explicitement le modèle "pay per token" pour développeurs.
   - Data Processing Addendum (`anthropic.com/legal/data-processing-addendum`) : Clauses Contractuelles Types de la Commission européenne (Module Two, controller-to-processor) incorporées par référence *"to the extent required by Applicable Data Protection Laws"* — s'applique donc automatiquement à un client européen, aucune signature séparée requise. Rôles explicites : *"Customer is the controller and Anthropic is Customer's processor"*.
   - Entraînement des modèles sur le contenu client explicitement exclu par défaut (Commercial Terms) : *"Anthropic may not train models on Customer Content from Services."*
   - Liste des sous-traitants d'Anthropic publiée à `trust.anthropic.com/subprocessors` (non consultée en détail — non bloquant pour la conclusion ci-dessus).

   Point retiré du §8.5 (ne nécessitait pas de validation professionnelle, juste une vérification factuelle sur sources publiques). Localisation exacte du projet Supabase toujours non trouvée dans le dépôt (le sous-domaine `*.supabase.co` est un identifiant de projet, pas une région) — reste à vérifier directement dans le dashboard Supabase.
...
| Anthropic (Claude) | Structuration IA du texte libre des paris | Texte libre saisi par l'utilisateur pour un pari (`lib/ai/structureBet.ts`) | Société américaine | Oui | DPA + clauses contractuelles types vérifiés (03/09/2026, voir §2.1 point 9) |
...
4. **Mécanismes de consentement** : décision retenue — **ajouter une déclaration d'âge simple au signup** (déclarative, sans justificatif, ex. case à cocher ou tranche d'âge plutôt qu'une date de naissance complète pour rester minimal cf. point 8). **Implémenté le 03/09/2026** : case à cocher "j'ai 15 ans ou plus" dans `SignupForm.tsx`, **bloquante** côté `lib/auth/actions.ts` (pas de compte créé sans confirmation, faute de mécanisme de consentement parental existant pour les moins de 15 ans) — décision au-delà de l'objectif initial de simple distinction, prise avec l'utilisateur au moment de l'implémentation. Horodatage en base via `users.age_confirmed_at` (migration `20260903120000_users_age_declaration.sql`).
...
7. **Modération et signalement** : décision retenue — **ajouter un signalement de message de chat** par les utilisateurs vers les admins, en complément de `bug_reports` qui ne couvre aujourd'hui que les rapports de bug technique, pas les messages problématiques. **Implémenté le 03/09/2026** : bouton "Signaler" par message (`ReportMessageButton.tsx`, visible sur tout message qui n'est pas le sien), snapshot du texte/auteur constitué côté serveur (jamais fait confiance au client), file de traitement admin dédiée `/admin/chat-reports` — table `chat_message_reports` (migration `20260903130000_chat_message_reports.sql`), même structure OPEN/RESOLVED que `bug_reports`.
8. **Limitation de la collecte** : la déclaration d'âge (point 4) devra rester minimale — trancher au moment de l'implémentation entre une simple case ("j'ai 15 ans ou plus") et une tranche d'âge, plutôt qu'une date de naissance complète non nécessaire à l'usage. **Tranché le 03/09/2026** : case à cocher retenue (pas de tranche d'âge), aucune date de naissance collectée.
...
**Résumé des décisions à implémenter (hors périmètre de ce cadrage, prochaine itération)** : ~~champ/case de déclaration d'âge au signup~~ (fait le 03/09/2026, voir point 4) ; ~~mécanisme de signalement d'un message de chat vers les admins~~ (fait le 03/09/2026, voir point 7).
...
- ~~Déclaration d'âge simple au signup (case ou tranche d'âge, sans justificatif)~~ : décidée en §2.10 point 4, **implémentée le 03/09/2026** (case bloquante dans `SignupForm.tsx`/`lib/auth/actions.ts`, colonne `users.age_confirmed_at`).
- ~~Signalement d'un message de chat vers les admins~~ : décidé en §2.10 point 7, en complément de `bug_reports` qui ne couvre pas ce cas — **implémenté le 03/09/2026** (table `chat_message_reports`, admin `/admin/chat-reports`).
- ~~Self-service suppression de compte + export de données~~ (décidé en §2.1 point 6) — **implémenté le 03/09/2026** :
  - *Suppression* : `lib/actions/account.ts` (`deleteAccountFormAction`), action serveur authentifiée, scopée à `user.id` lu de la session (jamais un paramètre client). Reprend l'ordre de suppression du script CLI (`scripts/delete-player-account.mjs`, conservé pour les cas que le self-service refuse) et ses garde-fous : blocage si rôle `ADMIN`, blocage si l'utilisateur a créé une ligue avec d'autres membres actifs. Confirmation UI par saisie du pseudo exact (`app/(app)/profile/page.tsx`, section « Zone de danger ») avant exécution. Couvre en plus `chat_message_reports` (table postérieure au script CLI, migration `20260903130000`), non gérée par celui-ci.
...
- ~~Transfert de données vers Anthropic (hors UE)~~ — **vérifié (03/09/2026), voir §2.1 point 9** : DPA incorporé automatiquement aux Commercial Terms d'Anthropic (couvrant tout usage API, pas seulement Enterprise), clauses contractuelles types incluses. Pas une validation professionnelle en attente — vérification factuelle sur sources publiques Anthropic, faite directement.
...
| Modération | Signalement d'un message par un utilisateur (§2.10 point 7, implémenté le 03/09/2026, table `chat_message_reports`) | Modération des échanges, protection des utilisateurs (dont mineurs) | Intérêt légitime | Supabase | Tant que le compte est actif | — |
