# NBA Pronos — Spécification Fonctionnelle V0.2

> **Statut : cadrage fonctionnel COMPLET ET CLOS (0.1 → 0.2.10, tous validés)**
> Ce document **remplace** la V0.1, ainsi que `nba_pronos_plan_cadrage_titres.md`, `nba_pronos_plan_cadrage_detaille.md` et `nba_pronos_resume_cadrage_valide.md`, devenus obsolètes ou partiellement dépassés par les décisions ultérieures.
...
> **Nouveauté V0.2** : intègre directement 5 derniers points ouverts fonctionnels tranchés (code compétition, bracket partiellement rempli, drill-down bracket, borne de l'écart, colonnes mobile du classement) — décisions actées en discussion mais jamais isolées dans un fichier `decisions_0_2_x` séparé. Le cadrage fonctionnel n'a plus de point ouvert non résolu en dehors des renvois à la spec technique.
...
```text
Inscription libre, accès immédiat, protégée par un CODE COMPÉTITION [VALIDÉ].
Nouvel inscrit = ACTIVE par défaut.
Admin peut passer un compte en DISABLED si nécessaire, et le réactiver.
```

Code compétition `[VALIDÉ]` :

```text
Inscription = pseudo + mot de passe + CODE COMPÉTITION → compte ACTIVE immédiatement.
```

- code **unique et partagé** pour toute la compétition (pas un code par joueur) ;
- demandé **une seule fois**, à l'inscription ;
- l'admin peut consulter / régénérer le code si besoin (fuite, nouvelle saison) ;
- un code invalide bloque l'inscription (message d'erreur explicite) ;
- reste un **filtre d'entrée léger**, cohérent avec l'esprit « confiance » du projet — il complète, sans le remplacer, le contrôle admin déjà prévu (validation, désactivation), et ne casse pas l'accès immédiat.
...
Bracket partiellement rempli à la deadline `[VALIDÉ]` :

```text
Traitement PAR SÉRIE, et non par bracket entier.
Série remplie (vainqueur + score)  → auto-validée normalement.
Série non remplie                 → absence sur CETTE série, 0 point, aucune pénalité.
Le reste du bracket (séries remplies) est traité normalement, sans invalidation globale.
```

Une série n'est considérée « remplie » que si les **deux** champs (vainqueur + score de série) sont renseignés — même règle d'indissociabilité que pour les pronos match (section 5.4). Cette règle applique au bracket la même logique déjà validée pour les pronos match (auto-validation du complet, absence du partiel), à la granularité de la série.
...
Vue publique par défaut = tendances agrégées ; un clic sur une série ouvre le détail de cette série (drill-down).

Rendu du drill-down `[VALIDÉ]` :

```text
Détail au clic sur une série = NOMINATIF COMPLET (qui a choisi quel vainqueur
+ quel score de série), pour CETTE série uniquement.
```

Cohérent avec le principe « aucun secret protégé après coup » déjà appliqué aux pronos match et aux paris personnalisés. Le bracket reste néanmoins moins exposé « brut » que les pronos match : on ne voit jamais le bracket complet d'un joueur d'un coup, uniquement série par série, à la demande.
...
Traitement du bracket partiellement rempli : voir section 4.1 (règle par série, `[VALIDÉ]`).
...
bornes 1 à 50 [VALIDÉ].
```

Borne haute = 50 `[VALIDÉ]` : garde-fou de **saisie** uniquement, sans aucun impact sur le scoring — le barème (section 7.2) attribue déjà un bonus d'écart plat à +0 dès 10 points d'écart et plus. Objectif unique : bloquer les erreurs de frappe manifestes sur le pavé numérique mobile.
...
Colonnes visibles sur mobile `[VALIDÉ]` :

```text
Mobile : 3 colonnes maximum par ligne — Rang/Joueur, Total, valeur de la puce
         de tri active (si différente du Total). Détail complet accessible
         via le DÉPLIABLE (section 8.4), jamais via scroll horizontal.
Desktop : toutes les colonnes affichées simultanément, sans contrainte.
```
...
remplis → %, sinon nombre brut). Clic sur une série = détail nominatif complet
(drill-down, [VALIDÉ], cf. section 4.5).
```
...
Mobile : 3 colonnes max (Rang/Joueur, Total, puce active) [VALIDÉ] ;
         détail via le dépliable, jamais de scroll horizontal.
...
| Bracket partiellement rempli à la deadline | Traitement par série : remplie = auto-validée, non remplie = absence (0 pt) sur cette série uniquement `[VALIDÉ]` |
...
Décision `[VALIDÉ]` : les 5 points auparavant listés comme « à trancher lors d'un futur cadrage fonctionnel » ont été résolus (voir sections 3.1, 4.1, 4.5, 5.2 et 8.3 / 11.5 ci-dessus). **Il ne reste donc plus aucun point ouvert relevant du cadrage fonctionnel** : seuls demeurent des points renvoyés à la spec technique ou classés non bloquants.

### 14.1 — Renvoyés à la spec technique (Claude Project dédié au développement)
...
### 14.2 — Non bloquants / reportés à une itération future
...
Cinq points fonctionnels supplémentaires (code compétition, bracket partiellement rempli, drill-down bracket, borne de l'écart, colonnes mobile du classement) ont été tranchés en discussion et intégrés directement ci-dessus (tagués `[VALIDÉ]`), sans fichier `decisions_0_2_x` séparé.
...
nba_pronos_SPEC_FONCTIONNELLE_V0_1.md   (remplacée par la présente V0.2)
```
...
```text
Cadrage fonctionnel : COMPLET ET CLOS.
Aucune décision produit/fonctionnelle ne reste à prendre dans cette conversation.

1.2 — Liste des points ouverts : intégrée à ce document (section 14),
      ne contient plus que des renvois techniques et des points non bloquants.
1.3 — Synthèse pour Claude Project (contexte optimisé pour le futur
      Claude Project dédié au développement) — À PRODUIRE.

Puis, une fois 1.3 produite :
