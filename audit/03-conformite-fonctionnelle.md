...
| Statut `POSTPONED` d'un match — traitement dans le moteur de scoring | `engine.ts` ne teste explicitement que `CANCELLED` pour neutraliser un score ; le comportement d'un match `POSTPONED` (reste `SCHEDULED`? passe par un statut dédié?) n'a pas été confirmé dans le code lu. | `lib/scoring/engine.ts` (agent business logic) | **À vérifier** — lire `lib/nba/client.ts` (`normalizeMatchStatus`) pour confirmer comment un report est effectivement représenté avant de conclure à un trou. |

## Synthèse

Le principal constat de cette phase n'est **pas** un manque de rigueur produit — c'est l'inverse : peu de projets de cette taille documentent aussi précisément leurs propres écarts à la spec. Le travail réel de cet audit sur cette phase a consisté à vérifier que les écarts déjà consignés dans `GAPS_OUVERTS.md` sont toujours d'actualité dans le code (échantillonnage confirmé cohérent) plutôt qu'à en découvrir de nouveaux. Les deux points neufs identifiés par cet audit et absents du journal existant sont le traitement du statut `POSTPONED` (à vérifier) et la mise en regard explicite du principe de prudence de structuration vs la confiance post-structuration (Catégorie 4).
